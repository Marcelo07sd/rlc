...
class LaplaceSolver:
    ...
    def solve(self, rlc_model, signal_type, signal_params):
        ...
        try:
            ...
            i_t = sp.simplify(i_t)
            # Redondear coeficientes numéricos para mejor legibilidad
            i_t = self._round_expression(i_t)
    ...
    def _round_expression(self, expr, decimals=2):
        """
        Redondea los coeficientes numéricos en una expresión simbólica
        ...
        Returns:
            Expresión con coeficientes redondeados
        """
        # Función para redondear números flotantes
        def round_floats(x):
            if isinstance(x, sp.Float):
                # Redondear según la magnitud
                val = float(x)
                if abs(val) < 0.01:
                    return x  # Mantener números muy pequeños como están
                elif abs(val) < 1:
                    return sp.Float(round(val, 4))
                elif abs(val) < 100:
                    return sp.Float(round(val, 2))
                else:
                    return sp.Float(round(val, 1))
            return x
        
        # Aplicar el redondeo a todos los elementos de la expresión
        return expr.replace(lambda x: isinstance(x, sp.Float), round_floats)